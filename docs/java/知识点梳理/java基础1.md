...
* \***可变参数**：\*typeName... parameterName作为方法的参数，前面是类型，后面是参数名，调用一个元素直接parameterName\[i]，一个方法最多只能有一个可变参数，必须放在参数列表最后
...
1. 集合：List/Set/Map、扩容、fail-fast、HashMap 原理。

2. 泛型：类型擦除、通配符、上下界、PECS。

3. 线程：线程生命周期、同步、线程池、并发容器。

4. IO/NIO：字节流、字符流、缓冲流、BIO/NIO。

5. 反射：Class 对象、构造方法、字段、方法调用、性能和安全边界。

6. 注解：元注解、运行时注解、编译期注解、APT。

7. 枚举：枚举字段、构造方法、抽象方法、单例。

8. Lambda/Stream/Optional：Java 8 现代基础。

9. 日期时间：Date/Calendar 的问题，java.time 的推荐用法。

10. 序列化：Serializable、transient、serialVersionUID、Android 中 Parcelable 对比。
