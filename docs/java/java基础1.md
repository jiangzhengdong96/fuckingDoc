...
* ***String：***
  ...
  **[常量池]()：**
  ...
  - **字符串常量池**：JVM 专门给 String 做的共享池
    ...
    ava 会维护一个专门的字符串共享机制，通常叫 StringTable。  字符串字面量，以及调用 intern() 后的字符串，会进入这里做复用。除了字符串常量池没有其它专门池
...
* **代码块（普通代码块、静态代码块、构造代码块、同步代码块）：**
  ...
  普通代码块：方法被调用，并且执行流走到它这里时才执行，不能脱离方法单独存在
  静态代码块：类初始化时执行一次，优先于对象创建，不能直接访问非静态成员，常用于初始化静态资源
  
  构造代码块：每次创建对象时，先于构造方法执行，每 new 一次执行一次，在构造方法之前执行，常用于提取多个构造方法共有初始化逻辑
